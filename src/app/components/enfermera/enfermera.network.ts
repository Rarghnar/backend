...
router.get("/all", async function(req: Request, res: Response){
    ...
    try {
        const result: Enfermera[] = await enfermeraController.getEnfermeras();
...
router.get("/:id", async function(req: Request, res: Response){
    ...
    try {
        const result: Enfermera | null = await enfermeraController.getEnfermeraById(id);
        responseModule.success(req,res,result);
...
router.post("/add", async function(req: Request, res: Response){
    ...
    const body: Enfermera = req.body;
    try {
        const result: Enfermera = await enfermeraController.addEnfermera(body);
...
router.delete("/delete", async function (req: Request, res: Response) {
    try {
      const ver = await enfermeraController.deleteEnfermera(req.body._id);
      if (ver != null) {
...
router.patch("/:id", async function(req: Request, res: Response){
    const { id } = req.params;
    try {
        const modPaciente = await enfermeraController.patchEnfermera(id, req.body);
